...
**TL;DR:** In Docker environment, Nodejs runs as a root user by default with unlimited permission set. Create a noon-root user during image creation and run the container with the flag "-u username"

**Otherwise:** An attacker who manage to run a script on the server gets unlimited power over the local machine (e.g. change iptable and re-route traffic to his server) He may change the iptable, 
