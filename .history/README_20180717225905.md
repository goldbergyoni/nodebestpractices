...
**TL;DR:** Though any step in the development chain should be protected with MFA (multi-factor authentication), NPM/Yarn are a sweet opportunity for attackers who guess or get a developer credentials to inject malicious code into libraries that are  widely installed across projects and Microservices. Maybe even across the web if published in puclic. Enabling 2 factor authentication in npm/YARN leaves almost zero chance for attackers to alter the packages code
