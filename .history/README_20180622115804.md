...
**TL;DR:** Untrusted data that is sent the browser might get executed locally instead of just being displayed. Disable this threat by escaping untrusted data (i.e. instructing the client to avoid interpretation of that data) using dedicated libraries that are telling the clients to treat this data as 'pure UI' that 
