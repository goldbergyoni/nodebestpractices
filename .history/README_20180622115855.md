...
**TL;DR:** Untrusted data that is sent down to the browser might get executed instead of just being displayed. Escaping untrusted data (i.e. instructing the client to avoid dainterpretation) using dedicated libraries that are telling the clients to treat this data as 'pure UI' that
