...
**TL;DR:** In Docker environment, Nodejs runs as a root user by default with unlimited permission set. Create a noon-root user during image creation and run the container with the flag Most base docker images pre-bake a non-root user "node" - run the containers with the flag "-u node"
