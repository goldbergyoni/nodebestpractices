...
According to the 'Principle of least privilege' a user/process must be able to access only the information and resources that are necessary. Granting root access to an attacker opens a whole new world of malicious ideas like routing traffic to other servers. Indeed, most Node apps don't need a root access and don't run with this priviliege. However, there are two common scenario that might push to root usage: (1) to gain access to priviliege port (e.g. port 80) Nodejs must run as root (2) Docker containers by default run as root(!). 
