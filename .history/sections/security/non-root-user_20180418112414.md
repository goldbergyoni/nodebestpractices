...
According to the 'Principle of least privilege' a user/process must be able to access only the information and resources that are necessary. Indeed, most Node apps don't need a root access and don't run with root user. There are two common  because of mainly two reasons: gain access to priviliege port (80)
