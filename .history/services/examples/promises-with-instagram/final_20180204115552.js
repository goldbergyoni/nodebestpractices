...
function tellMeWhatToEat(userId) {
  ...
  const images = getNearbyImages(instagramAPIURL).then(identifyFoodWithinImages);
...
function identifyFoodWithinImages(images) {
  ...
  images.data.forEach((mediaItem)=>{
    console.log(`About to analyze the image ${mediaItem.standard_resolution}`)
    app.models.predict(Clarifai.GENERAL_MODEL, mediaItem.standard_resolution)
    .then((response) => {
