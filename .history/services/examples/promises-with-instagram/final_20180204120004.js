...
function identifyFoodWithinImages(images) {
  ...
  images.data.forEach((mediaItem)=>{
    console.log(`About to analyze the image ${mediaItem.images.standard_resolution}`)
    app.models.predict(Clarifai.GENERAL_MODEL, mediaItem.images.standard_resolution)
    .then((response) => {
