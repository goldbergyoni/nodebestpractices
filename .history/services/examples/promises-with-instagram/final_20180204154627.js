...
function identifyFoodWithinImages(images) {
  ...
  images.data.forEach(mediaItem => {
    console.log(
      `About to analyze the image ${mediaItem.}`
    );
  });
...
function identifyFoodWithinImage(imageToFindFoodWithin) {
  const result = [];
  return new Promise((resolve, reject) => {
    app.models.predict(Clarifai.GENERAL_MODEL, imageToFindFoodWithin.images.standard_resolution.url).then(response => {
