...
function identifyFoodWithinImages(instagramPosts) {
  ...
  return new Promise((resolve, reject) => {
    instagramPosts.data.forEach(aPost => {
      postsArray.push(identifyFoodWithinImage(aPost));
    });
    return Promise.all(postsArray , (allFoodsFound) => {
...
function identifyFoodWithinImage(instagramPost) {
  const result = [];
  const app = new Clarifai.App({
    apiKey: "ae04f8d71fce4b8ab61168e6bbc5cd4d"
  });
  return new Promise((resolve, reject) => {
    app.models.predict(Clarifai.GENERAL_MODEL, instagramPost.images.standard_resolution.url).then(response => {
