...
**TL;DR:** 你应该采取预防措施来避免偶然地将机密信息发布到npm仓库的风险。 一个 `.npmignore` 文件可以被用作忽略掉特定的文件或目录, 或者一个在 `package.json` 中的 `files` 数组可以起到一个白名单的作用.

**否则:** 你项目的 API密钥、密码或者其它机密信息为被那些看到的人滥用而敞开大门，这可能会导致经济损失、身份冒充以及其它风险。
