...
## ![✔] 4.1 최소한 API (컴포넌트) 테스트는 써라

**핵심요약:** 대부분의 프로젝트는 짧은 일정 때문에 테스팅이 자동화되지 않거나 종종 ‘테스트 프로젝트’가 통제불능이 되거나 버려지곤 한다. 그러한 이유로, 유닛 테스트 보다 더 많은 적용 범위를 작성하거나 제공하는 가장 쉬운 방법인 API 테스트를 우선화하고 시작하여라. ([포스트맨](https://www.getpostman.com/)  같은 툴을 사용하지 않고도 API 테스트를 작성할 수 있다.) 더 많은 자원이나 시간을 가진 후에, 유닛 테스트, DB 테스트, 성능 테스트 등 같은 고급의 테스트 타입을 사용하라.

**그렇게 하지 않을 경우:** 시스템 적용 범위가 20% 밖에 안 된다는 것을 알아내기 위해 유닛 테스트를 작성하는데에 오랜 시간을 소비할 수 있다.

<br/><br/>

## ![✔] 4.2 각 테스트 이름에 3가지를 포함해라

**핵심요약:** 코드 내부에 익숙하지 않은 QA 엔지니어들과 개발자들에게 따로 설명이 필요 없도록 요구 사항 수준에서 테스트가 하려는 것을 말해라. 무엇을 테스트하고(테스트 하려는 유닛), 어떤 상황에서, 어떤 결과를 예상하고 테스트 하는 것인지 테스트 이름에 명시해라.


**그렇게 하지 않을 경우:** 배포에 실패하였습니다, “Add product”라는 이름의 테스트가 실패했습니다. 이런 메시지를 보고 정확히 뭐가 잘못되었는지 알 수 있는가?

🔗 [**자세히 보기: 각 테스트 이름에 3가지를 포함해라**](/sections/testingandquality/3-parts-in-name.md)

<br/><br/>

## ![✔] 4.3 AAA 패턴을 따라 테스트를 구축해라

**핵심요약:** 테스트를 다음 세 가지 부분으로 명확히 나누어라: 준비(Arrange), 실행(Act), 표명(Assert). 제일 먼저 테스트를 준비하고, 그 다음 테스트 하려는 유닛을 실행하고, 마지막은 확인 단계다. 이 구조를 따르는 것은 읽는이가 뇌 CPU를 쓰지 않고도 테스트 계획을 이해할 수 있도록 보장한다.

**그렇게 하지 않을 경우:** 하루종일 main 코드를 이해하는데 시간을 오래 쓸 뿐만 아니라 하루 중 간단한 부분(테스트)이어야 할 것이 뇌를 스트레칭한다. 


🔗 [**자세히 보기: AAA 패턴을 따라 테스트를 구축해라**](/sections/testingandquality/aaa.md)

<br/><br/>

## ![✔] 4.4 린터로 코드의 문제를 발견해라

**핵심요약:** 기본 품질을 확인하고 좋지 않은 패턴을 일찍 발견하기 위해 코드 린터를 사용해라. 테스트 하기 전에 린터를 실행하고 pre-commit git-hook을 추가해서 문제를 검토하고 수정하는 시간을 최소화한다. [섹션 3](#3-코드-스타일) 의 코드 스타일 사례들도 확인해라.

**그렇게 하지 않을 경우:** 일부 좋지 않은 패턴과 취약한 코드가 상용 환경에 넘어갈 수도 있다.

<br/><br/>

## ![✔] 4.5 공용 테스트 픽스처나 시드는 피하고, 테스트 별로 데이터를 추가해라

**핵심요약:** 테스트 결합도를 방지하고 테스트 흐름을 쉽게 추론하기 위해서, 각 테스트는 테스트용으로 DB 행(row) 집합을 추가하고 사용해야 한다. 테스트가 일부 DB 데이터를 가져와야하거나 존재한다고 가정해야할 때마다 명시적으로 데이터를 추가하고 다른 레코드의 변형을 피해야한다.

**그렇게 하지 않을 경우:** 테스트 실패로 배포가 중단되었다고 시나리오를 가정하자, 이제 슬픈 결론으로 끝나는 소중한 시간을 조사하는데에 소비할 것입니다.

🔗 [**자세히 보기: 공용 테스트 데이터를 피하라**](/sections/testingandquality/avoid-global-test-fixture.md)

<br/><br/>

## ![✔] 4.6 지속적으로 취약한 의존성을 검사해라

**핵심요약:** Express 같은 대부분 평판이 좋은 의존성도 알려진 취약점이 있다. 이는 모든 빌드마다 CI에서 호출될 수 있는 🔗 [npm audit](https://docs.npmjs.com/cli/audit)나 🔗 [snyk.io](https://snyk.io) 같은 커뮤니티나 상업용 도구를 사용하면 쉽게 길들일 수 있다.

**그렇게 하지 않을 경우:** 전용 도구 없이 취약점으로부터 깨끗한 코드를 유지하려면 새로운 위협에 관한 온라인 출판물들을 지속적으로 찾아야 할 것이다. 꽤 지루하다.

<br/><br/>

## ![✔] 4.7 테스트를 태그하라 (#테스트)

**핵심요약:** 서로 다른 테스트는 서로 다른 시나리오에서만 실행해야 한다. 빌드 성공 유무 테스트(quick smoke)나 입출력이 없는 테스트는 개발자가 파일을 저장하거나 커밋할 때 실행해야 하고, 전체 end-to-end 테스트는 종종 새로운 풀 리퀘스트가 전송될 때 실행해야 한다. 이는 #cold #api #sanity 같은 키워드로 테스트에 태그하여 수행할 수 있으므로 테스트 하네스(testing harness)에서 grep(텍스트 검색 기능)하고 원하는 하위 집합을 호출할 수 있다. 예를 들어, 이는 [모카](https://mochajs.org/)로 새너티 테스트(sanity test) 그룹만 호출하는 방법이다. mocha —grep 'sanity' 

**그렇게 하지 않을 경우:** 수십 개의 DB 쿼리를 수행하는 테스트를 포함한 모든 테스트를 실행하는 것은 개발자가 작은 변화를 만들 때마다 극도로 느려지고 개발자가 테스트를 실행하지 못하게 할 수 있다.
...
## ![✔] 5.14. 각 로그 문에 '트랜잭션 id'를 할당하라.

**핵심요약:** 단일 요청에서 각 로그 항목에 동일한 식별자(transaction-id: {some value})를 할당해라. 그러면 로그에서 에러를 검사할 때, 전후에 무슨 일이 발생했는지 쉽게 결론짓는다. 안타깝게도 이는 비동기적인 특성때문에 Node에서 구현하기 쉽지 않다. 내부 예제 코드 참조.

**그렇게 하지 않을 경우:** 전에 어떤 일이 발생했는지 맥락 없이 상용 에러 로그를 보는 것은 문제를 해결하는데 더 힘들고 오래 걸리게 한다.
...
**핵심요약:** 연구 결과에 따르면 자주 배포를 하는 팀이 상용 버전에서 심각한 에러가 발생할 가능성을 낮춘다고 한다. 위험이 따르는 수동적인 과정과 서비스 다운타임이 필요하지 않은 빠르고 자동화된 배포는 배포 프로세스를 크게 향상시킨다. 간소화된 배포의 표준이 된 Docker와 CI 도구를 결합하여 이를 달성할 수 있다.
...
**그렇게 하지 않을 경우:** 새로 발견된 버그나 취약점이 상용에서 운영중인 어플리케이션을 악용하는데 사용될 수 있으며, 다양한 모듈들에서 지원을 하지 않게 되고 유지보수하는 것이 힘들어 지게될 것이다
...
**핵심요약:** 로그의 목적지는 개발자가 어플리케이션 코드에 하드코딩해서는 안 되고, 대신 프로그램이 실행되는 실행환경에서 정의되어야 한다. 개발자는 로거 유틸리티를 이용하여 로그를 `stdout`에 작성하고 상용환경(컨테이너, 서버 등)이 해당 `stdout`스트림을 적절한 목적지(즉 Splunk, Graylog, ElasticSearch 등)로 수송해야한다
...
**핵심요약:** 상용환경에서 쓰는 코드가 테스트할 때 쓴 패키지 버전과 동일하다는 것을 반드시 보장해야 한다. package.json과 package-lock.json에 맞는 깔끔한 의존성 설치를 엄격하게 하기 위해 `npm ci`를 사용하라. 이 명령어는 지속적 통합 파이프라인과 같은 자동화된 환경에서 사용하는 것을 권장한다.

**그렇게 하지 않을 경우:** QA팀이 코드를 승인하기 전에 철저히 테스트해도 상용환경에서는 다르게 작동할 것이다. 게다가 같은 프로덕션 클러스터의 다른 서버들이 서로 다른 코드를 실행할 수도 있다.
...
**핵심요약:** R정확하게 수행되면 캐시로부터 전체 도커 이미지를 거의 즉각적으로 재구성 할 수 있다. 더 적게 업데이트된 지시사항은 Dockerfile 위쪽에 있어야 하고 앱 코드 같이 지속적으로 변하는 지시사항은 아래쪽에 있어야 한다.
...
## ![✔] 8.9. 명시적인 이미지 참조를 사용하고, `latest` 태그를 피해라.

**핵심요약:** 명시적인 이미지 다이제스트이나 버전 라벨을 명시하고 절대 `latest`는 참조하지 마라. 개발자들은 종종 `latest` 태그를 명시하는 것이 레퍼지토리에서 가장 최신의 이미지를 제공할 거라 믿지만 실제로 그렇지 않다. 다이제스트를 사용하는 것은 서비스의 모든 인스턴스가 같은 정확히 같은 코드를 실행할 수 있음을 보장한다.

게다가, 이미지 태그를 참조하는 것은 이미지 태그가 결정적 설치에 의존할 수 없어서 기본 이미지가 변경될 수 있음을 의미한다. 대신에, 결정적 설치가 필요하면 SHA256 다이제스트가 정확한 이미지를 참조하는데 사용될 수 있다.
...
## ![✔] 8.10. 더 작은 도커 기본 이미지를 선호하라

**핵심요약:** 큰 이미지는 더 많은 취약성 노출로 이어지고 자원 사용량을 증가시킨다. Slim과 Alpine Linux 변형과 같은 희박한 도커 이미지를 사용하는 것은 이러한 이슈를 완화한다.
