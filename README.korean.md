...
**핵심요약:** 기본 품질을 확인하고 좋지 않은 패턴을 일찍 발견하기 위해 코드 린터를 사용해라. 테스트 하기 전에 린터를 실행하고 pre-commit git-hook으로 추가해서 문제를 검토하고 수정하는 시간을 최소화한다. [섹션 3](#3-코드-스타일) 의 코드 스타일 사례들도 확인해라.
...
**그렇게 하지 않을 경우:** 전용 도구 없이 취약점으로 부터 깨끗한 코드를 유지하려면 새로운 위협에 관한 온라인 출판물들을 지속적으로 찾아야 할 것이다. 꽤 지루하다.
...
**핵심요약** 상용환경에서 쓰는 코드가 테스트할떄 쓴 패키지 버젼과 동일하다는 것을 반드시 보장해야 한다. package.json과 package-lock.json에 맞는 깔끔한 의존성 설치를 엄격하게 하기 위해 `npm ci`를 사용하라. 이 명령어는 지속적 통합 파이프라인과 같은 자동화된 환경에서 사용하는 것을 권장한다.
