...
**핵심요약:** 코드 내부에 익숙하지 않은 QA 엔지니어들과 개발자들에게 따로 설명이 필요 없도록 요구 사항 수준에서 이미 자명하도록 해라. 무엇을 테스트하고(테스트 하려는 유닛), 어떤 상황에서, 어떤 결과를 예상하고 테스트 하는 것인지 테스트 이름에 명시해라.
...
**핵심요약:** 기본 품질을 확인하고 좋지 않은 패턴을 일찍 발견하기 위해 코드 린터를 사용해라. 테스트 하기 전에 실행하고 pre-commit git-hook으로 추가해서 문제를 검토하고 수정하는 시간을 최소화한다. [섹션 3](#3-코드-스타일) 의 코드 스타일 사례들도 확인해라.
