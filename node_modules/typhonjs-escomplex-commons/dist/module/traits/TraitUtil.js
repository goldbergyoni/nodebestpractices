...
var _ASTParser = require('../../utils/ast/ASTParser');
...
var TraitUtil = function () {
   ...
   _createClass(TraitUtil, null, [{
      ...
      key: 'safeComputedName',
      value: function safeComputedName(node, parent) {
         var name = void 0;
         ...
         if (parent && parent.type === 'MethodDefinition') {
            if (typeof parent.computed === 'boolean' && parent.computed) {
               ...
               // `<computed>`.
               name = parent.key.type === 'Literal' ? TraitUtil.safeValue(parent.key) : '<computed~' + _ASTParser2.default.parse(parent.key).source + '>';
         ...
         if (node.type === 'ClassMethod') {
            if (typeof node.computed === 'boolean' && node.computed) {
               name = node.key.type === 'StringLiteral' ? TraitUtil.safeValue(node.key) : '<computed~' + _ASTParser2.default.parse(node.key).source + '>';
      ...
      key: 'safeComputedOperands',
      value: function safeComputedOperands(node, parent) {
         var operands = [];
      ...
      key: 'safeComputedOperators',
      value: function safeComputedOperators(node, parent) {
         var operators = [];

         // Handle ESTree case.
         if (parent && parent.type === 'MethodDefinition' && typeof parent.computed === 'boolean' && parent.computed) {
            operators.push.apply(operators, _toConsumableArray(_ASTParser2.default.parse(parent.key).operators));
         }

         // Handle Babylon AST ClassMethod node.
         if (node.type === 'ClassMethod' && typeof node.computed === 'boolean' && node.computed) {
            operators.push.apply(operators, _toConsumableArray(_ASTParser2.default.parse(node.key).operators));
         }
      ...
      key: 'safeNameOrValue',
      value: function safeNameOrValue(object) {
         var defaultNameOrValue = arguments.length <= 1 || arguments[1] === undefined ? '' : arguments[1];
         ...
         if (typeof defaultNameOrValue === 'string' && defaultNameOrValue !== '') {
            return defaultNameOrValue;
         }
