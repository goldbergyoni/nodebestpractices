...
class BaseDAL {
 ...
 //   }
  ...
  mapEntitiesToRows(entities, mappingToColumns) {
    let result = [];
    for(let i=0; i<entities.length;i++)
      result.push(this.mapEntityToRow(entities[i], mappingToColumns));
  ...
  async executeInTransaction(executeFunc){
    await this.knex.transaction(async (activeTransaction) => {
      await executeFunc(activeTransaction);
    });
